...
if __name__ == '__main__':
    ...
    # user_app_actived_count_df.to_csv('user_app_actived_count.csv', index=0)

    # 处理训练集和测试集
    trainData_X = age_train.iloc[:, 0:1]
    trainData_Y = age_train['age_group']
    testData_X = age_test.iloc[:, 0:1]

    # 根据id合并
    trainData_X = trainData_X.merge(user_app_actived_count_df, how='left', on='uid')
    trainData_X = trainData_X.fillna(axis=0,method='ffill')

    testData_X = testData_X.merge(user_app_actived_count_df, how='left', on='uid')
    testData_X = testData_X.fillna(axis=0,method='ffill')

    # 去掉 uid 列再训练
    uid = testData_X['uid']
    trainData_X = trainData_X.drop('uid', axis=1)
    testData_X = testData_X.drop('uid', axis=1)

    # trainData_X.to_csv('trainData_X.csv', index=0)
    # trainData_Y.to_csv('trainData_Y.csv', index=0)
    # testData_X.to_csv('testData_X.csv', index=0)

    # 得到训练集和测试集，便可释放大多数内存
    del age_train
    del age_test
    del app_category_dict
    del all_category_dict
    del all_category_name
    del user_app_actived_count
    del user_app_actived_count_df
    ...
    for index, (train_index, vali_index) in enumerate(skf.split(trainData_X, trainData_Y)):
        print(index)
        x_train, y_train, x_vali, y_vali = np.array(trainData_X)[train_index], np.array(trainData_Y)[train_index], np.array(trainData_X)[vali_index], np.array(trainData_Y)[vali_index]
        ...
        y_test = model.predict(testData_X,num_iteration=model.best_iteration)
